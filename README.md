...
### Zookeeper 功能设计
我们通过 curator-recipes 来初始化 zookeeper 客户端，并连接到 zookeeper 服务器：
```
// 重试策略: 初始休眠时间为 1000 毫秒（1秒），最多重试 3 次
...
## 程序实现流程
1. 扫描所有被注解的任务，并添加到全局变量中进行保存
2. 启动 zookeeper 客户端，并添加命令执行节点并监听，用来监听命令，控制后面的定时任务
3. 初始化并启动所有定时任务
4. 将所有定时任务挂载到节点上
5. 监听命令并执行命令，控制定时任务的开启/关闭
